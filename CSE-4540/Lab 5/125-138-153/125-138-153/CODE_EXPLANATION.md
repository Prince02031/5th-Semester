...
- `category`: Food category (enum: Fruits, Vegetables, Dairy, Meat, etc.)
- `quantity` & `unit`: Amount and measurement (e.g., 2 kg, 500 ml)
- `purchaseDate`: When item was bought
- `expiryDate`: When item expires (important for waste tracking!)
- `storageType`: Where stored (Refrigerator, Freezer, Pantry, Counter)
- `images`: Array of image URLs
- `consumptionCount`: How many times used (default: 0)
- `status`: Current state (Available, Partially Used, Consumed, Wasted, Expired)
...
- `reason`: Why it was wasted (Expired, Spoiled, Over-purchased, etc.)
...
- `getExpiringItems()`: Finds items expiring within 7 days

**`wasteLogController.js`:**
- `createWasteLog()`: Creates waste log AND updates grocery item status to 'Wasted'
- `getWasteAnalytics()`: Calculates total waste, waste by reason, waste by category

**`mealController.js`:**
- `getMealsByDateRange()`: Get meals between start and end dates

**`dailyNutritionLogController.js`:**
- `getDailyNutritionLogsByUser()`: Get all logs for specific user
